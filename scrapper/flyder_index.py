...
from flyder_parser import FlyderParser
...
class FlyderIndex:
    ...
    def get_num_pages(self, api_url, url, page_size=None):
        """ Use the showNumPages query
        to get the number of pages in the result set
        """
        query = {'url': url, 'showNumPages': True}
    ...
    def fetch_result_page(self, job_params):
        """ 
        ...
        # Get the result
        session = requests.Session()
        r = session.get(api_url, params=query, headers=req_headers, stream=True, timeout=timeout)
        ...
        if not gzipped:
            with open(filename, 'w+b') as fh:
                for chunk in r.iter_content(1024):
                    ## send to the parser ...
                    flyderParser = FlyderParser(chunk)
                    flyderParser.spread()
    ...
    def do_work(self, job_queue, counter=None):
        """ 
        ...
        while not job_queue.empty():
            try:
                ...
                logging.info('{0} page(s) of {1} finished'.format(num_done, job['num_pages']))
                ...
                if retries < job['max_retries']:
                    ...
                    logging.error('Max retries exceeded for page {0}'. format(job['page']))

    def run_workers(self, num_workers, jobs, shuffle):
        """ 
        ...
        for i in range(0, num_workers):
            tmp = Process(target=self.do_work,
    ...
    def read_index(self, r, api_url, prefix=None):
        logging.info('Getting Index From ' + api_url)
        logging.debug('Getting Num Pages...')

        num_pages = self.get_num_pages(api_url, r.url, r.page_size)
        ...
        if num_pages == 1:
            self.fetch_result_page(get_page_job(page_list[0]))
            return
        ...
        self.run_workers(num_workers, job_list, not r.in_order)


def get_args(instance):
    parser = ArgumentParser('CDX Index API Client')

    parser.add_argument('url', help="")
    parser.add_argument('-n', '--show-num-pages', action='store_true', help='Show Number of Pages only and exit')
    parser.add_argument('-p', '--processes', type=int, help='Number of worker processes to use')
    parser.add_argument('--fl', help='select fields to include: eg, --fl url,timestamp')
    parser.add_argument('-j', '--json', action='store_true', help='Use json output instead of cdx(j)')
    parser.add_argument('-z', '--gzipped', action='store_true', help='Storge gzipped results, with .gz extensions')
    parser.add_argument('-o', '--output-prefix', help='Custom output prefix, append with -NN for each page')
    parser.add_argument('-d', '--directory', help='Specify custom output directory')
    parser.add_argument('--page-size', type=int, help='size of each page in blocks, >=1')

    group = parser.add_mutually_exclusive_group()
    group.add_argument('-c', '--coll', help=('The index collection to use or "all" to use all available indexes. The default value is the most recent available index'))
    group.add_argument('--cdx-server-url', default=instance.CDX_SERVER_URL, help='Set endpoint for CDX Server API default to %s' % CDX_SERVER_URL)
    parser.add_argument('--timeout', default=30, type=int, help='HTTP read timeout before retry')
    parser.add_argument('--max-retries', default=5, type=int, help='Number of retry attempts')
    parser.add_argument('-v', '--verbose', action='store_true', help='Verbose logging of debug msgs')
    parser.add_argument('--pages', type=int, nargs='*', help=('Get only the specified result page(s) instead of all results'))
    parser.add_argument('--header', nargs='*', help='Add custom header to request')
    parser.add_argument('--in-order', action='store_true', help='Fetch pages in order (default is to shuffle page list)')
...
def main():
    ...
    for info in collinfo:
        instance.read_index(r, info['cdx-api'], info['id'])
